...
router.get('/list_document', async (req, res, next) => {
    try {
        console.log("i am hit theree")
        const folderPath = "./uploads/contractPdf"
        let getFiles = []
...
router.post('/checkout', authenticated, async (req, res, next) => {
    try {
        ...
        let find = await user_model.findOne({ _id: req.user })
        const file = await fs.readFile(`./uploads/agreement/${req.body.docs}`);
        ...
        fs.writeFile(`./uploads/agreement/${req.body.docs}`, pdfBytes);
        ///////
...
router.post('/register_landlord', async (req, res, next) => {
