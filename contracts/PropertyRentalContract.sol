...
contract PropertyRentalContract {
    ...
    function payRent(string calldata  month) external payable nonTenant allowedMonths(month) returns (bool success) {
        require(rentInStore[month].status == false, "Rent already paid for the given month"); // use require & not if statement, since function is payable & transaction should get reverted in invalid case.
    ...
    function withdrawRent(string calldata month) external onlyOwner allowedMonths(month) returns (bool success) {
        require(rentInStore[month].amount <= address(this).balance, "Insufficient contract balance to suffice the transaction"); // This is a must condition.
